"""Da modificare sulla base della teoria di Regularized OS-ELM,
modificare la classe regressore perchè le sue funzioni vengono utilizzate anche dal classificatore
Potrebbe essere opportuno aggiungere una nuova funzione di fit: _fit_regularized()
Per questo scopo sarà necessario modificare la classe per fare in modo che il nuovo fit venga richiamato 
...
from pyoselm_master.pyoselm.layer import MLPRandomLayer
...
class OSELMRegressor(BaseEstimator, RegressorMixin):
    ...
    def _fit_regularized(self, X, y):
        '''Leggi di aggiornamento'''
        H = self._create_random_layer().fit_transform(X)
        I = np.eye(safe_sparse_dot(H.T, H).shape[0])
        y = as_float_array(y, copy=True)
        if self.beta is None:
            # this is the first time the model is fitted
            if len(X) < self.n_hidden:
                raise ValueError("The first time the model is fitted, "
                                 "X must have at least equal number of "
                                 "samples than n_hidden value!")
            self.P = (1/self.C)*I + safe_sparse_dot(H.T, H)
            P_inv = pinv2(self.P)
