...
models = [[] for _ in range(12)]
BETAS = np.zeros((12, 12), dtype=object)
...
for rs_idx,rs in enumerate(rs_list):
    ...
    for Round in range(N_rounds):
        ...
        if Round == 0:
            ...
            beta_glob = aggregator(clients=clients, mode=agg_rule, scores=client_metrics, iteration=Round, Betas_in=BETAS)
            for j in range(len(clients)):
                for idx,client in enumerate(clients):
                    BETAS[j][idx] = beta_glob[idx]
            ...
            for idx, client in enumerate(clients):
                for j in range(len(clients)):
                    ...
                    BETAS[idx][j] = beta
            for idx,client in enumerate(clients):
                beta_glob = aggregator(clients=clients, mode='similarity_second', scores=client_metrics, iteration=Round, Betas_in=BETAS, index=idx)
                for j in range(len(clients)):
                    BETAS[idx][j] = beta_glob[j]
        metric_round = []
        metric_round_online = []
        # Testing on next training chunk
        '''for idx,client in enumerate(clients):
            model = OSELMRegressor(n_hidden=n_hidden, activation_func=activ_func, random_state=rs, C=C, beta=beta_glob[idx])
        ...
        metric_online[jojo].append(sum(metric_round_online)/len(metric_round_online))
        clients_metrics_online_std[jojo].append(np.std(metric_round_online))'''

        # Testing on the entire test-set
        for idx,client in enumerate(clients):
            model = OSELMRegressor(n_hidden=n_hidden, activation_func=activ_func, random_state=rs, C=C, beta=BETAS[0][idx])
